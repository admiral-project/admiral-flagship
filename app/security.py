...
def sanitize_error_message(error: Exception, context: str = "") -> str:
    ...
    if any(phrase in error_str for phrase in ["400", "bad request", "invalid", "malformed"]):
...
def init_security_headers(app):
    ...
    def add_security_headers(response):
        ...
        if os.environ.get("FLAGSHIP_SESSION_COOKIE_SECURE", "true").lower() in ("true", "1", "yes"):
...
def validate_production_config(config):
    ...
    if not is_production:
        ...
        if config.get("ADMIRAL_SHARED_TOKEN", "").startswith("dev-"):
            logger.warning("ADMIRAL_SHARED_TOKEN is using the development default; set it explicitly for production")
    ...
    # Check ADMIRAL_SHARED_TOKEN not dev default
    if config.get("ADMIRAL_SHARED_TOKEN", "").startswith("dev-"):
        errors.append("ADMIRAL_SHARED_TOKEN must not use development default in production")
