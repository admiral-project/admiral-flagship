...
class TestBFFCatalog:
    ...
    def test_save_app(self, client):
        with patch(
            "app.bff.catalog.api_post",
            _mock_api_post({"success": True, "name": "whoami"}),
        ):
            resp = client.post(
                "/flagship/api/catalog/apps/save",
                json={"yaml": "name: whoami\nversion: 1.0.0\n"},
            )
            assert resp.status_code == 200
    ...
    def test_edit_app_bumps_version(self, client):
        with patch(
            ...
            resp = client.post(
                "/flagship/api/catalog/apps/save",
                json={"app_id": "whoami", "yaml": "name: whoami\nversion: 1.0.0\n"},
            )
            assert resp.status_code == 200
    ...
    def test_disable_app(self, client):
        with patch(
            "app.bff.catalog.api_put",
            _mock_api_put({"success": True, "status": "inactive"}),
        ):
    ...
    def test_app_provisioning(self, client):
        with (
            patch(
                "app.bff.catalog.api_get",
                _mock_api_get({"id": "whoami", "name": "whoami"}),
            ),
            patch(
                "app.bff.catalog.api_get_text",
                lambda path: "name: whoami\ntiers:\n  starter:\n    cpu: 0.5\n    memory: 512Mi\n    storage: 5Gi\n    price_monthly: 9.99\n",
...
class TestBFFInstances:
    ...
    def test_instance_detail(self, client):
        with patch(
            "app.bff.instances.api_get",
            _mock_api_get({"id": "i1", "status": "running"}),
        ):
    ...
    def test_provision_instance(self, client):
        with patch(
            ...
            _mock_api_post({"operation_id": "op1", "status": "queued"}),
        ):
            resp = client.post(
...
class TestBFFBackupDetail:
    def test_backup_detail(self, client):
        with patch(
            "app.bff.backups.api_get",
            _mock_api_get({"id": "b1", "status": "completed"}),
        ):
...
class TestBFFBackupSettings:
    def test_backup_settings(self, client):
        with patch(
            "app.bff.backups.api_get",
            _mock_api_get({"storage_backend": "s3", "bucket": "admiral-backups"}),
        ):
...
class TestBFFBackupPrune:
    def test_prune_backups(self, client):
        with patch(
            "app.bff.backups.api_post",
            _mock_api_post({"success": True, "pruned_backups_count": 2}),
        ):
...
class TestBFFBackupVolumeTrigger:
    def test_trigger_volume_backup(self, client):
        with patch("app.bff.backups.api_post", _mock_api_post({"operation_id": "op1"})):
            resp = client.post(
                "/flagship/api/backups/trigger",
                json={"instance_id": "i1", "kind": "volumes"},
            )
            assert resp.status_code == 200
            assert resp.json["operation_id"] == "op1"
    ...
    def test_trigger_volume_backup_invalid_kind(self, client):
        resp = client.post(
            "/flagship/api/backups/trigger",
            json={"instance_id": "i1", "kind": "invalid"},
        )
        assert resp.status_code == 400
...
class TestBFFInstanceMigrate:
    ...
    def test_migrate_instance_accepts_legacy_target_node_id(self, client):
        with patch(
            ...
            resp = client.post(
                "/flagship/api/instances/i1/migrate",
                json={"target_node_id": "worker-02"},
            )
            assert resp.status_code == 200
            assert resp.json["operation_id"] == "op1"
